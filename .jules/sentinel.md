...
## 2025-02-28 - Unauthenticated unbounded Socket.io reports issue
**Vulnerability:** The `reportQuestion` socket endpoint appended incoming user-supplied JSON objects directly into `reports.json` without verifying authentication and without bounding the maximum array length.
**Learning:** This oversight allowed any unauthenticated user to spam requests, writing untrusted data and continually expanding the file until disk exhaustion.
**Prevention:** Socket.io endpoints that receive external payloads and write data must authenticate the socket session (e.g., `ensureAuthenticated(socket)`), rigorously extract and sanitize only expected fields, and enforce a strict upper bound length on stored arrays.
