...
window.startAiImport = async (pilotCount) => {
    ...
    // Read basic info (not the whole file for now to save socket size, ideally this would use a real upload endpoint)
    // For V1 socket integration we'll just mock the path and pass name/size
    const fileInfo = {
        name: file.name,
        size: file.size,
    ...
    window.socket.on("aiImportStatusData", (status) => {
        ...
        if(el) {
            ...
            if (s.review > 0) {
                ...
                // Note: Full review queue rendering will go here
            }
